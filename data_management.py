...
class DataManager:
    ...
    def add_books_simple(
        self, df: pd.DataFrame, isbn_list: list
            ...
            new_records.append(
                {
                    ...
                    "Status": (
                        "Found" if book_info.get("title") != "N/A" else "Not Found"
                    ),
                }
            )
    ...
    def save_to_excel(
        self, df: pd.DataFrame, filepath=Config.EXCEL_FILENAME, mode="append"
    ) -> bool:
        ...
        try:
            ...
            if mode == "append" and os.path.exists(filepath):
                ...
                logger.info(
                    f"✅ Appended to '{filepath}' - Total records: {len(combined_df)}"
                )
            else:
