...
from barcode_processing import get_book_info
import pandas as pd
...
def setup_dataframe(filename=Config.EXCEL_FILENAME) -> pd.DataFrame:
    """Create or load DataFrame from Excel file"""
    if os.path.exists(filename):
        df = pd.read_excel(filename)
        logger.info(f"📂 Loaded existing file '{filename}' with {len(df)} records")
        return df
    else:
...
def add_books_simple(df: pd.DataFrame, isbn_list: list) -> tuple[pd.DataFrame, int]:
...
def save_to_excel(df: pd.DataFrame, filename=Config.EXCEL_FILENAME, mode='append') -> bool:
    ...
    try:
        ...
        if mode == 'append' and os.path.exists(filename):
            ...
            logger.info(f"✅ Appended to '{filename}' - Total records: {len(combined_df)}")
        else:
            df.to_excel(filename, index=False)
            logger.info(f"✅ Saved to '{filename}' - Total records: {len(df)}")
