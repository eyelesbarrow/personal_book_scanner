...
class BarcodeScanner:
    """Main class to handle barcode scanning and book information retrieval
    Uses BarcodeProcessor for scanning and decoding, and manages the interactive scanning loop
    """

    barcode_processor = BarcodeProcessor()
    data_manager = DataManager()

    def interactive_scanner(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        ...
        while True:
            ...
            cv2.putText(
                frame,
                f"Scanned this session: {scanned_count}",
                (10, 60),
                Config.FONT,
                0.6,
                Config.GREEN,
            ...
            if key == ord(" "):  # SPACE key
                ...
                if ret:
                    ...
                    # Decode barcodes
                    result = self.barcode_processor.decode_barcodes(captured_frame)
                    ...
                    if isbn_list:
                        ...
                        # Add to database
                        result = self.data_manager.add_books_simple(df, isbn_list)
                        ...
                        if result is not None:
                            ...
                            cv2.putText(
                                error_frame,
                                "Try again with a clear barcode",
                                (50, 150),
                                Config.FONT,
                                0.7,
                                Config.WHITE,
    ...
    def single_scan_mode(self, df: pd.DataFrame) -> pd.DataFrame:
        """Single scan mode (original functionality)"""
        logger.info("Starting single scan mode...")

        image = self.barcode_processor.take_barcode_image(dev_mode=True)

        if image is not None:
            result = self.barcode_processor.decode_barcodes(image)
            ...
            if isbn_list:
                ...
                if result is not None:
                    df, added = result
                    if added > 0:
                        self.data_manager.save_to_excel(df, mode="append")
                        self.data_manager.display_dataframe_summary(df)
                else:
