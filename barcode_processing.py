...
def take_barcode_image(dev_mode=False) -> np.ndarray | None:
    ...
    if dev_mode:
        ...
        if barcode_frame is None:
            logger.error(f"Failed to load '{Config.TEST_IMAGE}'. Please ensure the file exists.")
            return None
...
def decode_barcodes(barcode_frame: np.ndarray) -> tuple[list, np.ndarray] | None:
        ...
        if is_isbn(barcode_data):
            ...
            if len(points) == 4: 
                ...
                cv2.putText(frame_with_boxes, text, 
                          ...
                          Config.FONT, 0.6, Config.WHITE, 2)
        else:
            ...
            if len(points) == 4:
                ...
                cv2.putText(frame_with_boxes, "Not ISBN", 
                          (pts[0][0], pts[0][1] - 10), 
                          Config.FONT, 0.5, Config.RED, 2)

    return isbn_list, frame_with_boxes

def is_isbn(barcode_data: str) -> bool:
...
def get_book_info(isbn: str) -> dict:
    ...
    try: 
        ...
        if book_key in data:
            ...
            if subjects:
                categories = [s.get("name", str(s)) if isinstance(s, dict) else str(s) for s in subjects[:3]]
